...
def start_bot(start_url, email, college, collegeID):
    ...
    if cap == 1:
        with open('2captoken.txt', 'r') as inputfile:
    ...
    if solved == 1:
        ...
        print(fc + sd + '[' + fm + sb + '*' + fc + sd + '] ' + fy + 'Step 1:' + fg + ' (Success)')
        fp = open('info.txt', 'a')
        ...
        fp.close()
        print(fc + sd + '[' + fm + sb + '*' + fc + sd + '] ' + fg + 'Details saved in info.txt, Starting Part 2...')
        ...
        print(fc + sd + '[' + fm + sb + '*' + fc + sd + '] ' + fy + 'Step 2:', end='')
        ...
        print(fc + sd + '[' + fm + sb + '*' + fc + sd + '] ' + fy + 'Waiting 30 seconds to submit...')
...
def main():
    ...
    with open('emails.txt', 'r') as inputfile:
      for line in inputfile:
        ...
        line = y[0]
        emailez.append(line)
    with open('college.txt', 'r') as inputfile:
      for line in inputfile:
        line.split(" ")
        inputcolleges.append(line)
    print(fc + sd + '[' + fm + sb + '*' + fc + sd + '] ' + fg + 'Selecting a college....\n')
    ...
    while isIDError != False:
        usv = 0
        for x in inputcolleges:
          usv = usv + 1
        renren = 0
        ranran = random.choices(inputcolleges, k = 1)
        for line in ranran:
            y = line.split("\n")
            line = y[0]
            renren = line
        userInput = int(renren)
        if userInput > len(allColleges) or userInput < 1:
            print(fc + sb + '[' + fm + sb + '*' + fc + sb + '] ' + fr + 'Wrong College id')
        else:
    ...
    print('\n' + fc + sb + '[' + fm + sb + '*' + fc + sb + '] ' + fw + sb + 'Selected College: ' + fc + sb + allColleges[userInput])

    time.sleep(0.4)
    emailman = random.choices(emailez, k=1)
    
    print('\n' + fc + sb + '[' + fm + sb + '*' + fc + sb + '] ' + fw + sb + 'Selected email: ' + fc + str(emailman[0]), end='')
    userEmail = str(emailman[0])
...
if __name__ == '__main__':
  while True:     
    try:
        user = int(input('' + fc + sb + "How many times would you like to apply: "))
        cap = int(input('' + fc + sb + "Would you like to use 2captcha? (1 = yes 2 = no): "))
        break  
    except ValueError:
        print('' + fr + sb + "Please enter an number!")

  for unused in repeat(None, int(user)):
    del unused
    main()